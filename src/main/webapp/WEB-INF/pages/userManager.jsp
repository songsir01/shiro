...
<link rel="<%=request.getContextPath()%>/jsp/public/stylesheet" type="text/css" href="style/css/index_1.css" />
</head>
		...
		<div id="TitleArea_Title">
			<div id="TitleArea_Title_Content">
				<img border="0" width="13" height="13"
					src="<%=request.getContextPath()%>/jsp/public/style/images/title_arrow.gif" />
				系统菜单-->用户管理
			</div>
		</div>
...
<!-- 		<button style="float: right;margin-left: 30px" onclick="toAdd()">添加</button> -->
		<shiro:hasPermission name="delete"><button style="float: right;margin-left: 30px" onclick="del()">删除</button></shiro:hasPermission>
		<shiro:hasPermission name="update"><button style="float: right;" onclick="toupd()">修改</button></shiro:hasPermission>
	</div>
	<!-- 主内容区域（数据列表或表单显示） -->
	<div id="MainArea">
			...
			<tbody id="TableData">
				<c:forEach items="${userList }" var="list">
					<tr class="TableDetail1" style="background: #DCDCDC"
						...
						<td align="center" style="font-size: small;"><c:forEach items="${list.roles }" var="role">${role.rname }</c:forEach></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								<input type="text" class="form-control" name="up_name"
									id="up_name" placeholder="" style="width: 55% ;text-align: center;" readonly="readonly">
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								...
								<input type="text" class="form-control" name="up_password"
									id="up_password" placeholder="默认不修改" style="width: 55%;text-align: center;ime-mode:disabled">
							</div>
						</div> 
						<div class="form-group">
...
<script type="text/javascript">
	...
	function goPage(){
		...
		location.href="<%=request.getContextPath()%>/userManager?page="+goPage+"&keyWord="+keyWord+"&pageSize="+pageSize+"&allPage="+allPage;
	}
	//选择
	function xuan(id){
...
/* 	function qxuan() {  
		...
		if(len==0){
			...
			$.ajax({
				...
				success:function(json){
					...
					$("#update").modal("toggle");
					for(var i in role){
						if(role[i].value==json.roles[0].rid){
	...
	function upStart(){
		...
		//alert(uid+" "+username+" "+password+" "+description+" "+sex+" "+rid);
		$.ajax({
